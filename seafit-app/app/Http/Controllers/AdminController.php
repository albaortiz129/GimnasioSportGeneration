...
class AdminController extends Controller
{
    ...
    public function changePlan(Request $request, User $user)
    {
        ...
        $nextPayment = $data['tarifa'] === 'cancelada'
            ? null
            : $this->siguienteCobro($data['tarifa']);
    ...
    public function manualCharge(Request $request, User $user)
    {
        ...
        $user->update([
            ...
            'next_payment_at' => $this->siguienteCobro($data['tarifa']),
    ...
    public function renewSubscription(User $user)
    {
        ...
        $user->update([
            'payment_status' => 'al_dia',
            'next_payment_at' => $this->siguienteCobro($user->tarifa, $base),
        ]);
     ...
     */
    public function clasesIndex(Request $request)
    {
        ...
        $clases = Clase::with([
            ...
            ->when($dia, fn($query) => $query->whereIn('dia_semana', $this->variantesDiaSemana($dia)))
        ...
        return view('admin.clases', compact('clases', 'usuarios', 'dia', 'q', 'diasSemana'));
    }
     ...
     */
    public function claseUpdate(Request $request, Clase $clase)
    {
        $data = $request->validate([
            'nombre' => 'required|string|max:255',
            'instructor' => 'required|string|max:255',
     ...
     */
    public function claseDestroy(Clase $clase)
    {
        $clase->delete();
    ...
    public function anadirUsuarioClase(Request $request, Clase $clase)
    {
        ...
        if ($clase->usuarios()->where('user_id', $user->id)->exists()) {
     ...
     */
    public function quitarUsuarioClase(Clase $clase, User $user)
    {
        if ($user->is_admin) {
            return back()->with('error', 'No se pueden gestionar administradores en clases.');
        }

        $existia = $clase->usuarios()->where('user_id', $user->id)->exists();
     ...
     */
    private function siguienteCobro(string $tarifa, ?Carbon $base = null): string
    {
        $fecha = ($base ?? now())->copy();
     ...
     */
    public function aprobarPagoManual(User $user)
    {
        if ($user->is_admin) {
            return back()->with('error', 'No aplica a administradores.');
        }
        ...
        $user->update([
            'payment_status' => 'al_dia',
            'next_payment_at' => $this->siguienteCobro($user->tarifa),
            'last_manual_payment_at' => now(),
        ]);
