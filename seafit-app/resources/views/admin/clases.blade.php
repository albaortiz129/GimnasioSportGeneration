...
@section('contenido')
    <div class="max-w-7xl mx-auto py-8 px-4">
        ...
        <form method="GET" action="{{ route('admin.clases.index') }}" class="bg-white border rounded-2xl p-4 mb-6">
        ...
        <div class="flex gap-2 overflow-x-auto mb-6">
            ...
            @foreach($diasSemana as $d)
                <a href="{{ route('admin.clases.index', ['dia' => $d, 'q' => $q ?? null]) }}"
            ...
            @foreach($clases as $clase)
                <div class="bg-white border rounded-2xl p-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <h3 class="font-bold mb-2">Apuntados ({{ $clase->usuarios->count() }})</h3>
                            @forelse($clase->usuarios as $u)
                                <div class="flex items-center justify-between text-sm border rounded p-2 mb-2">
                                    <span>{{ $u->nombre }} {{ $u->apellidos }}</span>
                                    <form action="{{ route('admin.clases.usuarios.destroy', [$clase, $u]) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <div>
                            ...
                            <form action="{{ route('admin.clases.usuarios.store', $clase) }}" method="POST" class="flex gap-2">
                                @csrf
                                <select name="user_id" class="border rounded p-2 w-full" required>
                                    @foreach($usuarios as $u)
                                        @if(!$clase->usuarios->contains($u->id))
                            ...
                            <form action="{{ route('admin.clases.destroy', $clase) }}" method="POST" class="mt-4"
