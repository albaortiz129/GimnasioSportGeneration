...
<div class="bg-[#fcfdfe] min-h-[80vh] flex justify-center items-center py-10 px-5">
    ...
    {{-- Tarjeta Login --}}
    <div class="bg-white w-full max-w-[450px] p-10 sm:p-12 rounded-[20px] shadow-[0_10px_40px_rgba(0,0,0,0.03)] text-center">
        ...
        <form action="{{ url('/login') }}" method="POST" class="text-left">
            ...
            <div class="mb-6">
                ...
                <div class="relative flex items-center">
                    ...
                    <span onclick="togglePass()" class="absolute right-4 cursor-pointer text-gray-400 hover:text-gray-600 transition-colors">
                        <span class="material-symbols-outlined text-[20px]">visibility</span>
                    </span>
                </div>
                
                <a href="{{ route('password.request') }}" class="block text-right text-[12px] text-[#004b7a] no-underline mt-2 font-semibold hover:underline">
                    ¿Olvidaste tu contraseña?
                </a>
            </div>

            {{-- Botón Enviar --}}
            <button type="submit" class="w-full bg-[#004b7a] text-white p-4 border-none rounded-xl text-[16px] font-bold cursor-pointer my-5 transition-colors duration-300 hover:bg-[#00385c]">
                Iniciar Sesión
            </button>
        </form>
        ...
        <p class="text-[13px] text-gray-500 m-0">
            ...
            <a href="{{ url('/registro') }}" class="text-[#004b7a] font-bold no-underline hover:underline">Regístrate aquí</a>
        </p>
    </div>
</div>
