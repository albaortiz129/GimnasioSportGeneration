    ...
    <div class="max-w-md w-full">
        ...
        <div class="bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.04)] border border-gray-100 overflow-hidden">
            ...
            <div class="p-8 sm:p-12">
                ...
                {{-- Mensaje de Éxito (El verde de tu captura, pero mejorado) --}}
                @if (session('status'))
                    <div class="mb-8 p-4 bg-green-50 border border-green-100 rounded-2xl flex items-center gap-3 animate-fade-in">
                ...
                <form action="{{ route('password.email') }}" method="POST" class="space-y-6">
                    @csrf
                    <div>
                        <label for="email" class="block text-xs font-black text-[#0A1931] uppercase tracking-widest ml-1 mb-2">
                            Correo Electrónico
                        </label>
                        <div class="relative">
                ...
                <div class="mt-10 text-center">
                    <a href="{{ route('login') }}" class="inline-flex items-center gap-2 text-sm font-bold text-gray-400 hover:text-[#1A3878] transition-colors group">
