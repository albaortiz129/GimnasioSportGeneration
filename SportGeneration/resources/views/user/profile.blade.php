...
@section('contenido')
    <div class="flex flex-col md:flex-row min-h-screen bg-[#f8fafc] font-sans">
        ...
        <main class="flex-1 p-6 md:p-10 lg:p-12 max-w-[1000px]">
            @if(session('success'))
                <div class="bg-[#c6ff7c] text-green-800 p-4 rounded-xl mb-6 border border-[#c6ff7c] font-medium">
                    {{ session('success') }}
                </div>
            @endif
            @if(session('error'))
            ...
            <section class="bg-white rounded-2xl p-6 md:p-8 mb-8 shadow-sm border border-gray-100">
                ...
                <div class="flex flex-col sm:flex-row gap-3 mt-6">
                    ...
                    <a href="{{ route('configuracion') }}"
                        class="inline-flex items-center justify-center gap-2 bg-white text-[#265e1f] border border-[#265e1f] px-5 py-3 rounded-xl font-bold hover:bg-blue-50 transition-colors">
