...
@section('contenido')
    <div class="max-w-md mx-auto my-10 p-8 bg-white rounded-3xl shadow-lg border">
        <h2 class="text-2xl font-bold mb-6 text-[#265e1f]">Añadir nueva tarjeta</h2>
        @if (!$stripeKey)
        ...
        <button id="card-button" data-secret="{{ $intent->client_secret }}"
            class="w-full bg-[#265e1f] text-white py-3 rounded-xl font-bold hover:bg-[#265e1f] transition-colors"
            @disabled(!$stripeKey)>
            Guardar tarjeta
        </button>
    </div>
