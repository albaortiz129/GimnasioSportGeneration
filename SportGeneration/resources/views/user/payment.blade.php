    ...
    <div class="flex flex-col md:flex-row min-h-screen bg-[#f8fafc] font-sans">
        ...
        <main class="flex-1 p-6 md:p-10 lg:p-12 max-w-[1000px]">
            <header class="mb-8">
                ...
                @if(session('success'))
                    <div
                        class="bg-[#c6ff7c] text-green-800 p-4 rounded-xl mb-4 border border-[#c6ff7c] font-medium flex items-center gap-2">
            ...
            <section class="bg-white rounded-2xl p-6 md:p-8 mb-8 shadow-sm border border-gray-100">
                ...
                @if($planActivo)
                    <div class="rounded-xl border border-[#c6ff7c] bg-[#c6ff7c] p-4">
            ...
            <section class="bg-white rounded-2xl p-6 md:p-8 mb-8 shadow-sm border border-gray-100">
                ...
                @foreach($metodosPago as $metodo)
                    ...
                    <div
                        class="border {{ $esPrincipal ? 'border-[#c6ff7c] bg-[#c6ff7c]' : 'border-gray-200 bg-[#f8fafc]' }} rounded-xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                        <div class="flex items-center gap-4">
                            <span class="material-symbols-outlined text-[32px] text-[#265e1f]">credit_card</span>
                            <div>
                                <h4 class="m-0 text-[#265e1f] font-bold text-lg">{{ ucfirst($metodo->card->brand) }} ****
                                    {{ $metodo->card->last4 }}
                                </h4>
                                <p
                        ...
                        <div
                            ...
                            @if(!$esPrincipal)
                                <form action="{{ route('pago.principal') }}" method="POST" class="m-0">
                                    ...
                                    <button type="submit"
                                        class="text-[#265e1f] bg-transparent border-none font-bold text-sm cursor-pointer p-0 hover:underline">
                ...
                @foreach($metodosManuales as $manual)
                    ...
                    <div
                        class="border {{ $esPrincipalManual ? 'border-[#c6ff7c] bg-[#c6ff7c]' : 'border-gray-200 bg-[#f8fafc]' }} rounded-xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                        <div class="flex items-center gap-4">
                            <span class="material-symbols-outlined text-[32px] text-[#265e1f]">payments</span>
                            <div>
                                <h4 class="m-0 text-[#265e1f] font-bold text-lg">{{ $manual['label'] }}</h4>
                                <p
            ...
            <section class="bg-white rounded-2xl p-6 md:p-8 mb-8 shadow-sm border border-gray-100">
                <div class="border-b border-gray-100 pb-4 mb-5">
                    <h3 class="text-xl font-bold text-[#265e1f]">Historial de facturas</h3>
                </div>
                ...
                <div class="flex flex-col gap-5">
                    @forelse($user->invoices() as $factura)
                        ...
                        <div
                            ...
                            <div>
                                <h4 class="m-0 text-[#265e1f] font-bold">Factura {{ $factura->date()->format('M Y') }}</h4>
                                <p class="m-0 mt-1 text-sm text-gray-500">{{ $factura->date()->format('d/m/Y') }} | Pago
                                    Completado</p>
                            </div>
                            <div class="flex items-center gap-4 sm:gap-6">
                                <span class="font-black text-[#265e1f] text-lg">{{ $factura->total() }}</span>

                                <a href="{{ route('factura.descargar', $factura->id) }}"
                                    class="text-[#265e1f] font-bold text-sm flex items-center gap-1.5 hover:text-[#265e1f] transition-colors bg-white px-3 py-1.5 rounded-lg border border-gray-200 shadow-sm">
                ...
                <section class="bg-white rounded-2xl p-6 md:p-8 mb-8 shadow-sm border border-gray-100">
                    ...
                    <form action="{{ route('pago.cambiar_plan_metodo') }}" method="POST"
                        ...
                        <button class="bg-[#265e1f] text-white rounded p-3 font-bold">Actualizar</button>
                    </form>
                </section>
